...
void main() {
  ...
  for (final barrier in ['schema', 'config']) {
    for (final invalidation in [
      ...
      test('adversarial $barrier fence $invalidation exact resource owner', () async {
        ...
        final a = configureAcceptedNativeVoiceSession(
          ...
          isStillCurrent: () =>
              currentProfile == '' && identical(currentConnection, connection),
        ...
        try {
          ...
          switch (invalidation) {
            ...
            case 'owner':
              expect(voice.cancelNativeVoicePreparationOwnedBy(owner), isTrue);
            case 'supersession':
              expect(
